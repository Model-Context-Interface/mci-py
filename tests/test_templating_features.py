...
class TestBasicTemplatingFeatures:
    ...
    def test_api_url_templating(self):
        ...
        result = self.engine.render_basic(url_template, context)
        assert result == "https://api.weather.com/weather?location=New York&units=metric"

    def test_auth_header_templating(self):
        ...
        result = self.engine.render_basic(header_template, context)
        assert result == "Bearer sk_live_123456789"

    def test_request_body_templating(self):
        ...
        result = self.engine.render_basic(body_template, context)
        assert result == '{"user": "alice", "email": "alice@example.com", "age": 30}'

    def test_file_path_templating(self):
        ...
        result = self.engine.render_basic(path_template, context)
        assert result == "/var/reports/report-2024-Q1.pdf"

    def test_cli_command_templating(self):
        ...
        result = self.engine.render_basic(cwd_template, context)
        assert result == "/home/user/projects/myapp"

    def test_nested_object_templating(self):
        ...
        result = self.engine.render_basic(template, context)
...
@endforeach"""
    ...
    def test_numbered_list_with_for(self):
    ...
    def test_conditional_error_message(self):
        ...
        result = self.engine.render_advanced(template, context_success)
        assert "✓ Operation completed" in result

        # Error case
        context_error = {"props": {"status": "error"}, "env": {}, "input": {}}
        result = self.engine.render_advanced(template, context_error)
...
Mode: Development
@endif"""
        result = self.engine.render_advanced(template, context_prod)
...
Generated by {{env.GENERATED_BY}}"""
    ...
    def test_dynamic_email_template(self):
...
class TestTemplatingEdgeCases:
    ...
    def test_placeholder_at_end(self):
        ...
        result = self.engine.render_basic("Text at {{props.value}}", context)
    ...
    def test_adjacent_placeholders(self):
        ...
        result = self.engine.render_basic("{{props.first}}{{props.second}}", context)
        assert result == "HelloWorld"

    def test_foreach_with_empty_array(self):
    ...
    def test_for_loop_with_zero_range(self):
    ...
    def test_if_with_nonexistent_path(self):
    ...
    def test_missing_placeholder_raises_error(self):
        ...
        with pytest.raises(TemplateError) as exc_info:
            self.engine.render_basic("{{props.missing}}", context)
...
class TestInputAlias:
    ...
    def test_input_and_props_are_same(self):
        ...
        result1 = self.engine.render_basic(template1, context)
        result2 = self.engine.render_basic(template2, context)
        assert result1 == result2 == "Alice"

    def test_mixed_input_and_props_usage(self):
